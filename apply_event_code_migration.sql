...
BEGIN
    IF NOT EXISTS (
        ...
        -- Add constraint to ensure event_code format
        ALTER TABLE events ADD CONSTRAINT events_event_code_format 
        CHECK (event_code IS NULL OR (event_code ~ '^[A-Z0-9]{6}$'));
        
        RAISE NOTICE 'event_code column added successfully';
    ELSE
        RAISE NOTICE 'event_code column already exists';
    END IF;
END $$;

-- Function to generate event codes for existing events
...
BEGIN
    ...
    RAISE NOTICE 'Generated event codes for % existing events', 
        (SELECT COUNT(*) FROM events WHERE event_code IS NOT NULL);
END;
$$ LANGUAGE plpgsql;
