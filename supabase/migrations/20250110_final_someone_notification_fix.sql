...
-- This fixes the respond_to_event_invitation function that's still creating "Someone" notifications

-- 1. Fix the respond_to_event_invitation function
...
-- 2. Drop and recreate the send_event_invitations_to_crew function to include proper inviter name
...
COMMENT ON FUNCTION send_event_invitations_to_crew IS 'FINAL VERSION: No more "Someone" notifications. Uses comprehensive user name fallback for inviter names.';

-- 5. Test message
SELECT 'All database functions fixed - no more "Someone" notifications will be created!' as status;
