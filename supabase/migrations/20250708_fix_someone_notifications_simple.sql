-- SIMPLE FIX: Just fix the main respond_to_event_invitation function
-- This should resolve the "Someone" notification issue

-- 1. Ensure respond_to_event_invitation is completely fixed
...
SELECT 
    'respond_to_event_invitation function updated!' as status,
    'This should fix both "Someone" notifications and notification updates' as description;

-- Test query to see if there are still functions with "Someone"
SELECT 
    routine_name,
    CASE 
        WHEN routine_definition ILIKE '%Someone%' THEN '❌ Still has "Someone"'
        ELSE '✅ Clean'
    END as status
