-- COMPREHENSIVE FIX: Eliminate duplicate notifications and fix user data issues
-- This migration fixes all the notification issues identified:
-- 1. Removes duplicate notification creation (trigger vs RPC function)
-- 2. Fixes "Someone" notifications with proper user data fallback
-- 3. Standardizes notification formats
-- 4. Ensures consistent user name display
...
-- Step 2: Fix the respond_to_event_invitation function to eliminate "Someone" notifications
...
-- Step 3: Fix the send_event_invitations_to_crew function with proper user names
...
-- Step 4: Create a function for individual user invitations (used by bulkInviteUsers)
-- This ensures consistent notification creation for all invitation types
-- First drop any existing versions of this function
...
GRANT EXECUTE ON FUNCTION create_event_invitation_notification TO authenticated;
...
COMMENT ON FUNCTION create_event_invitation_notification IS 'NEW FUNCTION: Creates consistent event invitation notifications for individual user invitations';
