-- EMAIL-NOTIFICATION SYNCHRONIZATION FIX
-- This migration ensures that when users respond to invitations via email,
-- the corresponding in-app notifications are automatically updated to reflect the response

-- Step 1: Update the process_event_invitation_token function to sync notifications
...
-- Step 2: Update the process_crew_invitation_token function to sync notifications
...
-- Step 5: Test message
SELECT 'Email-notification synchronization implemented! Email responses will now automatically update in-app notifications.' as status;
